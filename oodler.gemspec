# Generated by jeweler
# DO NOT EDIT THIS FILE DIRECTLY
# Instead, edit Jeweler::Tasks in Rakefile, and run the gemspec command
# -*- encoding: utf-8 -*-

Gem::Specification.new do |s|
  s.name = %q{oodler}
  s.version = "0.1.5"

  s.required_rubygems_version = Gem::Requirement.new(">= 0") if s.respond_to? :required_rubygems_version=
  s.authors = ["Mike Hagedorn"]
  s.date = %q{2010-04-16}
  s.description = %q{Ruby wrapper for the Oodle API}
  s.email = %q{mike@silverchairsolutions.com}
  s.extra_rdoc_files = [
    "LICENSE",
     "README.markdown"
  ]
  s.files = [
    "LICENSE",
     "README.markdown",
     "Rakefile",
     "VERSION",
     "lib/oodler.rb",
     "lib/oodler/category.rb",
     "lib/oodler/client.rb",
     "lib/oodler/element.rb",
     "lib/oodler/image.rb",
     "lib/oodler/location.rb",
     "lib/oodler/oodle_response.rb",
     "lib/oodler/region.rb",
     "lib/oodler/source.rb",
     "lib/oodler/web_client.rb",
     "test/client_test.rb",
     "test/element_test.rb",
     "test/fixtures/blank.xml",
     "test/fixtures/fail.xml",
     "test/fixtures/listings.xml",
     "test/image_test.rb",
     "test/test_helper.rb",
     "test/web_client_test.rb"
  ]
  s.homepage = %q{http://github.com/mwhagedorn/oodler}
  s.rdoc_options = ["--charset=UTF-8"]
  s.require_paths = ["lib"]
  s.rubygems_version = %q{1.3.5}
  s.summary = %q{Ruby wrapper for the Oodle API}
  s.test_files = [
    "test/client_test.rb",
     "test/element_test.rb",
     "test/image_test.rb",
     "test/test_helper.rb",
     "test/web_client_test.rb",
     "examples/listings.rb"
  ]

  if s.respond_to? :specification_version then
    current_version = Gem::Specification::CURRENT_SPECIFICATION_VERSION
    s.specification_version = 3

    if Gem::Version.new(Gem::RubyGemsVersion) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<roxml>, ["~> 3.1.3"])
      s.add_runtime_dependency(%q<crack>, ["~> 0.1.4", "<= 0.1.6"])
      s.add_runtime_dependency(%q<httparty>, [">= 0.5.2"])
      s.add_development_dependency(%q<shoulda>, [">= 2.10.1"])
      s.add_development_dependency(%q<jnunemaker-matchy>, ["= 0.4.0"])
      s.add_development_dependency(%q<mocha>, ["= 0.9.4"])
      s.add_development_dependency(%q<fakeweb>, [">= 1.2.5"])
    else
      s.add_dependency(%q<roxml>, ["~> 3.1.3"])
      s.add_dependency(%q<crack>, ["~> 0.1.4", "<= 0.1.6"])
      s.add_dependency(%q<httparty>, [">= 0.5.2"])
      s.add_dependency(%q<shoulda>, [">= 2.10.1"])
      s.add_dependency(%q<jnunemaker-matchy>, ["= 0.4.0"])
      s.add_dependency(%q<mocha>, ["= 0.9.4"])
      s.add_dependency(%q<fakeweb>, [">= 1.2.5"])
    end
  else
    s.add_dependency(%q<roxml>, ["~> 3.1.3"])
    s.add_dependency(%q<crack>, ["~> 0.1.4", "<= 0.1.6"])
    s.add_dependency(%q<httparty>, [">= 0.5.2"])
    s.add_dependency(%q<shoulda>, [">= 2.10.1"])
    s.add_dependency(%q<jnunemaker-matchy>, ["= 0.4.0"])
    s.add_dependency(%q<mocha>, ["= 0.9.4"])
    s.add_dependency(%q<fakeweb>, [">= 1.2.5"])
  end
end

